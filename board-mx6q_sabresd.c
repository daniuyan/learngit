...
#define SABRESD_MIPI_INT	IMX_GPIO_NR(2, 7)
...
#define SABRESD_GT911_RST      IMX_GPIO_NR(2, 16)
#define SABRESD_GT911_INT      IMX_GPIO_NR(6, 8)
#define SABRESD_GT928_INT      IMX_GPIO_NR(4, 20)
#define SABRESD_GT928_RST      IMX_GPIO_NR(6, 7)
...
#define SCREEN_MIPI     BACKLIGHT_PWM3 
#define SCREEN_LVDS     BACKLIGHT_PWM3 
#define SCREEN_LCD      BACKLIGHT_PWM1 
#define SCREEN_UNKNOWN  0

#define CODEC_WM8960    0
#define CODEC_WM8962    1
...
static struct platform_device mx6_sabresd_audio_tef6635_device = {
	.name = "imx-tef6635",
};
...
static struct i2c_gpio_platform_data pdata = {
   .sda_pin                = SABRESD_SD2_WP,
   .sda_is_open_drain      = 0,
   .scl_pin                = SABRESD_AUX_3V15_EN,
   .scl_is_open_drain      = 0,
   .udelay                 = 2,
};
...
static struct platform_device *hsi2c_gpio_devices[] __initdata = {
	  &hsi2c_gpio_device,
};
...
static void gt928_reset(int addr)
{
       	...
       	gpio_direction_output(SABRESD_GT928_RST, 0); // begin select I2C slave addr
       	msleep(20);                         // T2: > 10ms
       	// HIGH: 0x28/0x29, LOW: 0xBA/0xBB
       	gpio_direction_output(SABRESD_GT928_INT, addr == 0x14); // set addr
       	msleep(2);                          // T3: > 100us
       	gpio_direction_output(SABRESD_GT928_RST, 1); // end select I2C slave addr
...
static void gt911_reset(int addr)
{
        ...
        gpio_direction_output(SABRESD_GT911_RST, 0); // begin select I2C slave addr
        msleep(20);                         // T2: > 10ms
        // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
        gpio_direction_output(SABRESD_GT911_INT, addr == 0x14); // set addr
        msleep(2);                          // T3: > 100us
        gpio_direction_output(SABRESD_GT911_RST, 1); // end select I2C slave addr
	msleep(20);
...
static struct i2c_board_info mxc_i2c3_board_info[] __initdata = {
    	{	
		I2C_BOARD_INFO("ft5326_ts", 0x38),
		.irq = gpio_to_irq(SABRESD_MIPI_INT),
	},
};

static struct i2c_board_info mxc_i2c0_board_info[] __initdata = {
	{
		I2C_BOARD_INFO("wm8960-codec", 0x1a),
	},
	{
...
static struct i2c_board_info mxc_i2c2_board_info[] __initdata = {
	...
	{
               	I2C_BOARD_INFO("Goodix-TS-gt928", 0x5d),
               	.platform_data = (void *)&gt928_pdata,
	},
#if CONFIG_MXC_CAMERA_OV5640_MIPI
   {
...
#if CONFIG_FB_MXC_TRULY_WVGA_SYNC_PANEL
    {
	  .disp_dev = "mipi_dsi",
	  .interface_pix_fmt = IPU_PIX_FMT_RGB666,
	  .mode_str = "TRULY-WVGA",
...
#if CONFIG_FB_MXC_EK79007_WVGA_SYNC_PANEL
    {
	  .disp_dev = "mipi_dsi",
	  .interface_pix_fmt = IPU_PIX_FMT_RGB24,
...
{
	/* 注册tef6635设备，6635无需在此处初始化 */
	mxc_register_device(&mx6_sabresd_audio_tef6635_device,
                                   &tef6635_data);
	...
	mxc_register_device(&mx6_sabresd_audio_sgtl5000_device,
									&sgtl5000_data);
	imx6q_add_imx_ssi(0, &mx6_sabresd_ssi_pdata);
	
	//添加三个sgtl5000的控制函数，该开关量是在imx6_defconfig中默认打开的
	#ifdef CONFIG_SND_SOC_SGTL5000
...
static void gps_power_on(bool on)
{
	...
	//gpio_request(SABRESD_AUX_3V15_EN, "aux_3v15_en");//bluesky
	//gpio_direction_output(SABRESD_AUX_3V15_EN, 1);//bluesky
	//gpio_set_value(SABRESD_AUX_3V15_EN, on);//bluesky
	//gpio_free(SABRESD_AUX_3V15_EN);//bluesky
	...
	for_each_tag(t, tags) {
  			...
  			/* touchscreen and pwm device select */
                        ...
                        if (str != NULL) {
							   ...
							   if(strncmp(str,"lcd",3)==0)
							   {
                                       screen_type0=SCREEN_LCD;
                               }
                               else if(strncmp(str,"mipi_dsi",8)==0)
							   {
                                       screen_type0=SCREEN_MIPI;
							   }
                        }
                          /* touchscreen and pwm device select */
                        str = t->u.cmdline.cmdline;
                        ...
                        if (str != NULL) {
							   ...
							   {
                                       screen_type1=SCREEN_MIPI;
						       }

                        }
						  /* codec select */
				        str = t->u.cmdline.cmdline;
						str = strstr(str, "audio_codec=");
						if (str != NULL) {
						        str += 12;
							    if(strncmp(str,"wm8960-11M",10) == 0){
								    audio_codec = CODEC_WM8960;
									wm8960_clk_rate = 11289600;
								}
							    else 
                                if(strncmp(str,"wm8960-12M",10) == 0){
								    audio_codec = CODEC_WM8960;
								    wm8960_clk_rate = 12288000;
								}
								else if(strncmp(str,"wm8960-24M",10) == 0){
									audio_codec = CODEC_WM8960;
									wm8960_clk_rate = 24000000;
							    }
                                else
								{
								    audio_codec = CODEC_WM8962;
								}
					    }
                        break;
		}
...
static void __init mx6_sabresd_board_init(void)
{
	...
	*/
...
#ifdef CONFIG_MXC_CAMERA_OV5640
	imx6q_add_v4l2_capture(0, &capture_data[0]);//ov5640csi
#endif
#if CONFIG_MXC_CAMERA_OV5640_MIPI
	imx6q_add_v4l2_capture(1, &capture_data[1]);//ov5640mipi
#endif
	...
	{
		strcpy(mxc_i2c0_board_info[0].type, "wm8960-codec");
	}
   ...
   i2c_register_board_info(3, mxc_i2c3_board_info,
	...
	mx6_sabresd_pwm_backlight_data.pwm_id = screen_type0 | screen_type1;
